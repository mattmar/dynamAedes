...
## Load time series of daily average temperature
setwd("~/GitHub/euaeae")
library(actuar)
w <- read.csv('t_data.csv')
...
pld <- readRDS('ld_matrix.RDS')
row.names(pld) <- which(w[,1]%in%row.names(pld))
...
space <- nrow(ww)
## Time
days <- ncol(ww) #n of day to simulate
## Vector of propagules to initiate the life cycle
e.intro.n <- rep(0,space); e.intro.n[sample(1:space,1)] <- 0 #n of introduced eggs
i.intro.n <- 0 #n of introduced immatures
a.intro.n <- rep(0,space); a.intro.n[sample(as.integer(colnames(pld)),1)] <- 50 #n of introduced adults

### Life cycle ###
if( exists("counter")) rm(counter)
	for (day in 150:300) {
		if( !exists("counter") ) {
			counter <- 0; i.surv.m <- matrix(0,ncol=5,nrow=2); i.temp.v <- 0; e.temp.v <- 0; a.egg.n <- 0; p.life.a <- array(0,c(3,nrow(ww),5)); outl <- list()
...
## Derive daily rate for gonotrophic cycle,i.e. blood meal to oviposition. 
		a.gono.r <- a.gono_rate.f(ww[,day]/1000)
...
## Derive oviposition rate, i.e., number of eggs laid per female per day 
		a.batc.n <- a.ovi_rate.f(ww[,day]/1000)
## Adult survival
		source("./lc/a.surv_rate.f.r")
## Derive daily adult female survival rate
		a.surv.r <- a.surv_rate.f(ww[,day]/1000)
...
## Probability of long passive dispersal (from Pasaoglu et al. 2012)
		f.pdis.p <- rgamma(seq(1,max(pld,na.rm=T),1000),shape=16/(10000/16), scale=10000/16)
## Probability of egg survival
		e.surv.p <- 0.99
## Probability of hatching (from Soares-Pinheiro et al. 2015)
		e.hatc.p <- exp(-rgamma(length(ww[,day]/1000),shape=(7.6/7.4)^2,rate=(7.6/7.4^2)))
...
# A has five sub-compartments representing: adults in 1,2 of oviposition[2:3]; 2d+ old adults host-seeking and non ovipositing[4]; 2d+ old adults which digested the blood meal but not yet laying[1]; 1d old adults, non-laying and non-dispersing[5].
		...
		if( any(which(p.life.a[3,,4]>0)) ) {
			...
			for (i in 1:length(f.ocel.v)) {
				...
				a.plan.l <- sapply((which(f.adis.v[,i]<max(asd[,e]))*10-10), function(x) {if(x<250) dx<-as.numeric() else dx<-asd[e,as.numeric(which(asd[,e]==x))]})
				#Set dispersing distances which do not exist in the distance matrix asd as NULL
				a.plan.l <- lapply(a.plan.l, function(x) if(length(x)>0&is.null(names(x))) x=numeric() else x)
				# Randomly choose one of the landing cells for each dispersing distance
				a.land.v <- sapply(a.plan.l, function(x) {if(length(x)>0) sample(x,1,replace=TRUE) else -999})
				toret <- as.integer(which(a.land.v>0)) 
				if( length(f.adis.v[which(f.adis.v[,i]<max(asd[e,])),i][toret])>0 ) {
					# Remove dispersing individuals from cell of origin
					p.life.a[3,e,4] <- p.life.a[3,e,4] - sum(f.adis.v[which(f.adis.v[,i]<max(asd[e,])),i][toret])
					# Add dispersing individuals in the chosen landing cell for each distance
					p.life.a[3,as.integer(names(a.land.v[toret])),4] <- p.life.a[3,as.integer(names(a.land.v[toret])),4] + f.adis.v[which(f.adis.v[,i]<max(asd[e,])),i][toret]
					#print(p.life.a[3,,1:5][unique(which(p.life.a[3,,1:5]>0,arr.ind=T)[,1]),unique(which(p.life.a[3,,1:5]>0,arr.ind=T)[,2])])
...
## Long-distance passive dispersal
	# It happens only if any cell with at least one adult is in proximity of roads; therefore it is key that colnames(pld) and the order of cells correspond to p.life.a
		if( any(which(p.life.a[3,,]>0)%in%colnames(pld)) ) {
		# Extract cells whose contain long-distance dispersing adults
			f.opac.n <- unique(which(p.life.a[3,,]>0,arr.ind=T)[,1])[which(unique(which(p.life.a[3,,]>0,arr.ind=T)[,1])%in%colnames(pld))]
		...
		#Place dispersing adults in landing cells and remove them from origin cells
			if( length(landing_d)>0 ) {
				...
				# Select landing cells along road corresponing to the dispersing distance and subset at random to a single landing cell 
						ll <- as.integer(colnames(pld)[which(pld[,which(colnames(pld)%in%f.opac.n[op])]%in%(landing_d[[op]]*1000))])
						if( length(ll)>0 ){
...
# Make a new host-seeking compartment and slide ovipositing female status for new day
		...
		message("\nday ",length(counter)," has ended\n")
# If the sum of all compartments and cells is 0, the population is exinct, stop the life cycle.
		if(sum(p.life.a,na.rm=T)==0) stop("Exinct!")
	}

### Plot results ###
#trend in time
outlp<-mclapply(outl, function(x) apply(x, MARGIN=c(1, 2), sum),mc.cores=4)
...
for (i in 1:(ncol(wadults)-2)) {
	...
	text(x=4210000,y=2380000,as.Date(i+150, origin = "2015-01-01"),col="red",cex=2)
	...
	text(x=4270000,y=2380000,labels=paste("Total number of adults",sum(wadults[,i+2]),sep=": "),col="black",cex=2)
	#Sys.sleep(0.1)
}	