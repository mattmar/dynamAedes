...
# Run simulations, start 15th of April 2017; end 31 October 2019
str=165; endr=1037; ie=100; nc=5; it=5
source("zanzinv_sa.r")
zanzout <- zanzinv(temps.matrix=ww,cells.coords=cc,road.dist.matrix=pld,startd=str,endd=endr,n.clusters=nc,cluster.type="MPI",iter=it,intro.cells=introvector,intro.eggs=ie,sparse.output=FALSE,compressed.output=TRUE,country="es",suffix=paste("out_alg_sd",str,"_ed",endr,"_i",it,"_e",ie,sep=""))

#temps.matrix=ww; cells.coords=cc; road.dist.matrix=pld;startd=135; endd=1037; n.clusters=2; cluster.type="SOCK" ; iter=2; intro.cells=NA; intro.eggs=100
...
citoplot$date<-as.Date(citoplot$day,origin="2017-04-15")
...
#scale_x_date(lim=c(as.Date("2019-01-01"),as.Date("2019-12-01")),date_breaks="1 month",date_labels="%b-%y")
scale_x_date(date_breaks="1 month",date_labels="%b");g1
