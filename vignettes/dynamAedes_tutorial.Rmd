...
it = 2 # The higher the number of simulations the better
...
psi(input_sim = simout, eval_date = 62)
```
...
```{r}
r=adci_sp(simout, coords=cc, eval_date=123, breaks=c(0.5,0.975), stage=3)
plot(r)
...
```{r}
x=icci(simout, eval_date=1:123, breaks=c(0.25,0.50,0.75))
head(x)
tail(x)
```
...
```{r}
x=dici(simout, coords=cc, eval_date=seq(1,123,length.out=123), breaks=c(0.25,0.50,0.75), space=FALSE)
