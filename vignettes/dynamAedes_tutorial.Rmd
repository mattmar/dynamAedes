...
invasive *Aedes* mosquitoes. Users can apply the stochastic, time-discrete and spatially-explicit population dynamical model initially developed in Da Re et. al 2021 (DOI: https://doi.org/10.1016/j.ecoinf.2020.101180) for *Aedes aegypti* and then expanded for other three species: *Ae. albopictus*, *Ae. japonicus* and *Ae. koreicus*
...
We will describe applications of the model for *Ae. albopictus* and for all spatial scales by using a simulated temperature dataset.
...
At punctual scale, the model only requires a weather station temperature time series provided as a numerical matrix (in degree Celsius). For the purpose of this tutorial, we simulate a 3-year long temperature time series.

## Simulate temperature data with seasonal trend
We first simulate a three-year temperature time series with seasonal trend. For the time series we consider a mean value of 18°C and standard deviation of 3°C.
...
### Run the model
Running the model with the settings specified in this example takes about 10 minutes.
...
```
## Analyse the results
Model output is saved as an RDS file in the working directory or can be accessed from the R environment.
```{r, echo=FALSE, results='hide',message=FALSE}
...
So if we inspect the first iteration, we observe that the model has computed 366 days, as we had specified above in the object *endr*.
...
So if we inspect the 1st and the 50th day within the first iteration, we obtain a matrix having
```{r}
dim(simout[[1]][[1]])
simout[[1]][[1]]
simout[[1]][[50]]
...
```{r, eval=FALSE, echo=TRUE}
psi(input_sim = simout, eval_date = 300)
```
...
The "local scale" means that the model accounts for both active and passive dispersal of the mosquitoes. With this setting, the model requires three input datasets: a numerical temperature matrix (in degree Celsius) defined in space and time (space in the rows, time in the columns), a two-column numerical matrix reporting the coordinates (in meters) of each space-unit (cell) and a numerical *distance matrix* which reports the distance in meters between the cells connected through a road network. For the purpose of this tutorial, we will use the following simulated datasets:
  ...
  2. A 3-year long spatially and temporally correlated temperature time series; 
...
After defining the road segment we add a "buffer" of 100 m around the road segment. Adult mosquitoes that reach or develop into cells comprised in the 100 m buffer around roads are thus able to undergo passive dispersal. 
```{r}
...
### Run the model
Running the model with the settings specified in this example takes about 15 minutes.
```{r, eval=FALSE, echo=TRUE}
simout=dynamAedes(species="albopictus",
            scale="lc",  
            ihwv=habitat_liters,  
...
The **second** level corresponds to the simulated days.
So if we inspect the first iteration, we observe that the model has computed 731 days, as we had specified above in the object *endr*.
...
iterations that resulted in a viable mosquito population at a given date.
```{r}
psi(input_sim = simout, eval_date = 730)
```
...
```{r}
plot(psi_sp(coords = cc, input_sim = simout, eval_date = 730, n.clusters=cl))
```
At local scale, the interpretation of this output has a more nuanced interpretation: a pixel having psi=0 can be a pixel where all the simulations resulted in an extinction or where the species has not yet arrived through dispersal. 
...
We can also have a spatial output of the estimated interquantile range abundance of a given life stage using the function *adci_sp* just specifiying the pixels coordinates. For example for eggs: 
...
```{r}
x=icci(simout, eval_date=1:731, breaks=c(0.25,0.50,0.75))
head(x)
tail(x)
```
...
The **second** level corresponds to the simulated days.
If we inspect the first iteration, we observe that the model run for 366 days, as we had specified above in the object *endr*.
...
```{r}
plot(psi_sp(coords = cc, input_sim = simout, eval_date = 100,n.clusters=cl))
```
### Derive abundance 95% CI for each life stage and in each day
