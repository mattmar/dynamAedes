...
We then add a spatial pattern into the lattice area. This spatial pattern will be used later to add spatial correlation (SAC) to the temperature time series. 
...
We generate a spatially autocorrelated raster adding the SA variable (*xyz$sim1*) to the SpatRaster object. The autocorrelated surface could for example represent the distribution of vegetation cover in a urban landscape.
```{r}
...
## 1.3 Simulate an arbitrary road segment for medium-range dispersal 
...
So if we inspect the first iteration, we observe that the model has computed 60 days, as we had specified above in the object *endr*.
...
```{r}
psi.output <- psi_sp(coords = cc, input_sim = simout, eval_date = 60, n.clusters=cl)
```
...
```{r}
albo.disp <- dici(simout, coords=cc, 
