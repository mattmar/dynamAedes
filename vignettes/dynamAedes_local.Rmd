---
title: "Describing the main features of *dynamAedes*: the local scale model."
subtitle: "dynamAedes: A Unified Mechanistic Model For The Population Dynamics Of Invasive *Aedes* Mosquitoes."
...
date: "`r Sys.Date()`"
vignette: >
  %\VignetteIndexEntry{Describing the main features of *dynamAedes*: the local scale model.}
...
This tutorial explains step-by-step the main features of **dynamAedes** package, a unified modelling framework for invasive *Aedes* mosquitoes. Users can apply the stochastic, time-discrete and spatially-explicit population dynamical model initially developed in [Da Re et al., (2021)](https://doi.org/10.1016/j.ecoinf.2020.101180) for *Aedes aegypti* and then expanded for other three species: *Ae. albopictus*, *Ae. japonicus* and *Ae. koreicus* [Da Re et al., (*under review*)](http://dx.doi.org/10.1101/2021.12.21.473628).
...
```{r}
r=adci_sp(simout, coords=cc, eval_date=30, breaks=c(0.025,0.975), stage=3)
plot(r)
...
```{r}
x=icci(simout, eval_date=1:30, breaks=c(0.25,0.50,0.75))
head(x)
tail(x)
```
