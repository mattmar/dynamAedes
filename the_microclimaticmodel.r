...
nedem50<-get_dem(dem1kmr,resolution=50,zmin=1)
...
temps <- microclima::runauto(nedem1km, dstart="01/01/2016", dfinish="31/12/2016", hgt = 1,	l = NA, x = NA, r.is.dem=TRUE, habitat = 14, plot.progress = FALSE, save.memory=TRUE, dailyprecip=prcp,coastal=TRUE,use.raster=TRUE,zmin=1)
...
for(i in day){
	...
	dailyT <- append(dailyT,resample(raster(Tday,template=nedem100),nedem250, method='bilinear'))
}
