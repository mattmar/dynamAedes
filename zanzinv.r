...
zanzinv <- function(temp.matrix=ww,cell.dist.matrix=asd,road.dist.matrix=pld,startd=1,endd=10,n.clusters=1,cluster.type="SOCK",iter=1,intro.adults=100,intro.larvae=0,intro.eggs=0) {
	...
	sapply("libraries", function(x) {assign(x,get(x),envir= .GlobalEnv)})
	## Load packages
	suppressPackageStartupMessages(ipak(c("foreach","doSNOW","Rmpi","actuar")))
	## Type of cluster
	...
	rs <<- foreach(iteration=1:iter) %dopar% {
		...
		if( exists("counter") ) rm(counter)
			foreach(day = startd:endd, .combine=c) %do% {
				if(!stopit) {
					if( !exists("counter") ) {
						counter <- 0; i.surv.m <- matrix(0,ncol=5,nrow=2); i.temp.v <- 0; e.temp.v <- 0; a.egg.n <- 0; p.life.a <- array(0,c(3,nrow(temp.matrix),5)); outl <- list()
					...
					a.gono.r <- a.gono_rate.f(temp.matrix[,day]/1000)
					...
					a.batc.n <- a.ovi_rate.f(temp.matrix[,day]/1000)
					## Adult survival
					source("./lc/a.surv_rate.f.r")
					## Derive daily adult female survival rate
					a.surv.r <- a.surv_rate.f(temp.matrix[,day]/1000)
					...
					i.surv.r <- i.surv_rate.f(temp.matrix[,day]/1000)
					## Transform rate in daily probabiltiy to survive.
					i.surv.p <- exp(-i.surv.r)
					## Immature emergence
					source("./lc/i.emer_rate.f.r")
					## Derive daily immature emergence rate
					i.emer.r <- i.emer_rate.f(temp.matrix[,day]/1000)
					## Transform rate in daily probabiltiy to survive.
					...
					e.hatc.p <- exp(-rgamma(length(temp.matrix[,day]/1000),shape=(7.6/7.4)^2,rate=(7.6/7.4^2)))
					...
					if( any(which(p.life.a[3,,4]>0)) ) {
						...
						for (i in 1:length(f.ocel.v)) {
							...
							a.plan.l <- sapply((which(f.adis.v[,i]<max(cell.dist.matrix[,e]))*10-10), function(x) {if(x<250) dx<-as.numeric() else dx<-cell.dist.matrix[e,as.numeric(which(cell.dist.matrix[,e]==x))]})
							...
							if( length(f.adis.v[which(f.adis.v[,i]<max(cell.dist.matrix[e,])),i][toret])>0 ) {
								# Remove dispersing individuals from cell of origin
								p.life.a[3,e,4] <- p.life.a[3,e,4] - sum(f.adis.v[which(f.adis.v[,i]<max(cell.dist.matrix[e,])),i][toret])
								# Add dispersing individuals in the chosen landing cell for each distance
								p.life.a[3,as.integer(names(a.land.v[toret])),4] <- p.life.a[3,as.integer(names(a.land.v[toret])),4] + f.adis.v[which(f.adis.v[,i]<max(cell.dist.matrix[e,])),i][toret]
							#print(p.life.a[3,,1:5][unique(which(p.life.a[3,,1:5]>0,arr.ind=T)[,1]),unique(which(p.life.a[3,,1:5]>0,arr.ind=T)[,2])])
