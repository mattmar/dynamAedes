	...
	if(!dispersal) message("\n ### Model without dispersal ### \n") 
		...
		rs <- foreach( iteration=1:iter, .packages="foreach", .export = ls(globalenv()) ) %dopar% {
			...
			# Index for dynamic array adults
					...
					if(species!="aegypti") p.life.a[4,,2:4] <- apply(t(p.life.a[4,,1:3]),MARGIN=mrg,function(x) rbinom(size=x,n=space,prob=d.surv.p))
					...
					if(species!="aegypti") d.temp.v <- p.life.a[4,,4] - if(species!="aegypti") d.hatc.n else 0
