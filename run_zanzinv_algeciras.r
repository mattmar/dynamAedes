...
w <- readRDS('data/algeciras_temps11-19_corr.RDS')[,c(1:3,((365*6)+3):3229)] #select years 1=2011; 365*6 means you start from 1 Jan 2017
...
# Run simulations
source("zanzinv.r")
zanzout <- zanzinv(temps.matrix=ww, cells.coords=cc, road.dist.matrix=pld,startd=175,endd=510,n.clusters=4, cluster.type="SOCK",iter=4,intro.cells=introvector,intro.eggs=100,sparse.output=FALSE,compressed.output=TRUE)
...
citoplot$date<-as.Date(citoplot$day,origin="2017-05-25")

w <- readRDS('~/GitHub/euaeae/data/algeciras_temps11-19_corr.RDS')[,c(((365*6)+3):3229)][,175:(174+days)]
...
scale_y_continuous(sec.axis = ~ ./100); g1

ggsave("~/algeciras_500.png",g1,dpi=400,scale=1.5,width=25,height=15,unit="cm")

###Spatial spread
w <- readRDS('data/algeciras_temps11-19_corr.RDS')[,c(1:3,((365*7)+3):3229)] #select years 1=2011;
...
plotdistt<-cbind.data.frame(returndis(zanzred,days=335),day=1:335)
