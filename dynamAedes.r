...
## ---------------------------------------------------##
## Latest updated and tested: November 2020
## ---------------------------------------------------##
## Scope: Population dynamical model for `Aedes` spp.
## ---------------------------------------------------##
		...
		foreach(day = startd:endd, .combine=c) %do% {
			if( !stopit ) {
				if( !exists("counter") ) {
					...
					counter <- 0; i.surv.m <- matrix(0,ncol=5,nrow=2); i.temp.v <- 0; d.temp.v <- 0; e.temp.v <- 0; a.egg.n <- 0; a.degg.n <- 0; p.life.a <- array(0,c(4,nrow(temps.matrix),6)); storage.mode(p.life.a) <- "integer"; outl <- list()
				...
				a.surv.p <- a.surv_rate.f(temps.matrix[,day]/1000)
                ## Immature emergence
				...
				d.hatc.n <- rbinom(length(1:space), p.life.a[4,,4], e.hatc.p)
				...
				e.temp.v <- rbinom(length(1:space), e.temp.v, 0.99)
				d.temp.v <- rbinom(length(1:space), d.temp.v, 0.99)
